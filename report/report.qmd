...
All'interno della direttiva di OpenMP si possono usare delle clausole che permetto o di ottimizzare il proprio programma o in alcuni casi di renderlo funzionanante.
Queste clausole modificano il comportamento della direttiva principale specificandocome i dati devono essere gestiti, come i thread devono interagire e come il carico di lavoro deve essere distribuito.
...
Facendo il test con una seconda PCU con una dimensione della cache minore si ottengono i seguenti risultati:
...
In alcuni il livello di parallelizzazione e le prestazioni ottenute dalla CPU possono essere insufficuenti per alcune applicazioni, una alternativa puo' essere usare MPI su di un supercomputer solo che questa opzione e' difficilmente giustificabile, quindi una terza opzione e' quella di usare hardware specifico come possono essere le GPU.
...
Come si poteva prevedere la GPU in questo test batte le CPU nettamente, la questione e' che dipende dal tipo di calcolo che si sta facendo, nel caso dei frattali di Lyapunov viene eseguito essenzialmente il calcolo di un logaritmo un' innumerevole numero di volte quindi un caso affine alla GPU se il calcolo fosse qualcosa che richide elevata precisione, come ad esempio operazioni floting point, il distacco fra CPU e GPU sarebbe minore.

# Conlusioni 

Da tutti i test fatti si riesce chiaramente vedere che la parallelizzazione tramite OpenMP, nel caso dei frattali di Lyapunov, conviene in ogni caso in paritcolare usando OpenMP quello detto e' ancora piu' vero essendo che l'impementazione e' semplicemente l'aggiunta di una direttiva all'interno del codice.

Anche se tramite OpenMP non si riece ad arrivare alle massime prestazini che si possono avere in assoluto, si vede chiaramente dai test fatti che a livello di prestazioni ne vale la pena, ma soprattutto conviene usare OpenMP per la poca intrusivita' all'interno del codice infatti nei casi piu' semplici basta solamente qualche linea di codice. 
