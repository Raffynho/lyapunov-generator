---
title: "Generatore di Frattali di Lyapunov con Parallelizzazione tramite OpenMP" 
...
L'obbettivo del progetto e' quello di esplorare l'ottimizzazione, per il tempo di esecuzione, di un programma sequenziale attraverso l'utilizzo delle direttive di OpenMP cercando di dimostrare la sublinearita' del programma parallelizzato.
...
Un frattale e', intuitivamente, una figura in cui un singolo motivo viene ripetuto su scale descrescenti, come si può vedere osservando la curva di Von Koch: 
...
In genere quando si compra una CPU una delle caratteristiche prese in considerazione e' il numero di **Cores** o **Threads** perche' un maggiore numero permette di fare piu' cose.
...
Essendo abbastanza difficoltoso eseguire il programma centinaia di volte variando i parametri di test, all'interno del **Makefile** sono configurati dei comandi che permetto di fare tutto cio' molto facilmente.
...
![Sequenziale](images/serial.png){#fig-sequenziale width="2.8in"}

![Parallelo](images/parallel.png){#fig-parallelo width="2.8in"}

Immagini generate con le due versioni del programma
:::

Eseguendo lo script sulle due immagini sopra (@fig-labeled) si ottiene un' errore massimo di:
...
Come si puo' vedere dal risultato dello script le due immagini sono perfettamente identice.

## Ottimizzazioni 

All'interno della direttiva di OpenMP si possono usare delle clausole che permettono di ottimizzare il proprio programma o in alcuni casi di renderlo funzionanante.
Queste clausole modificano il comportamento della direttiva principale specificando come i dati devono essere gestiti, come i thread devono interagire e come il carico di lavoro deve essere distribuito.

### Collapse

Supponiamo ad esempio di dover fare una operazione su di ogni punto all'interno di una griglia bidimensionale, naturamente nel programma ci troveremo in una situazione del genere:
...
Per parallelizzare questo codice possiamo aggiungere il comando **#pragma omp parallel for** della direttiva di **OpenMP** come mostrato nel blocco sopra ci permette di parallelizzare il ciclo for, solo pero' quello esterno, questo significa che nel caso in esempio ad ogni thread viene assegnata un' intera riga della griglia.
...
Facendo il test con e senza collapse otteniamo:
...
bind_rows(df_parallel_stats, df_collapse_stats) %>%
  ...
  labs(
    title = "Confronti con e senza clausola Collapse",
    x = "Numero di Thread",
...
I tipi di schedule all'interno della direttiva di **OpenMP** sono:
...
ggplot(df_schedule_stats, aes(x = Threads, y = mean_time, color = Schedule, group = Schedule)) +
  ...
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper)) +
  labs(
    title = "Confronto fra tipi di Schedule",
    x = "Numero di Thread",
    y = "Tempo Medio di Esecuzione [s]",
...
Dal grafico si vede chiaramente che tutti i tempi di esecuzione per ogni tipo di schedule si sovrappongono quindi non fa differenza il tipo di schedule, la migliore opzione sara' quella di non spcificare alcun tipo di schedule (di default e' **dynamic**) in modo da diminuire l'overehead creato dalla direttiva.

#### Chunk size

Scegliendo come tipi di schedule **static** o **dynamic** un fattore che si puo' modificare e' la dimensione dei chunk.


```{r echo=F, message=F, warning=F}
df_chunk <- read_csv("data/benchmark_results_chunk.csv") %>% 
  mutate(
...
df_chunk_stats %>%
  ggplot(aes(x = Chunk_Size, y = mean_time, color = "#F8776D")) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper)) +
  scale_x_continuous(breaks = df_chunk_stats$Chunk_Size) +
  theme(legend.position = "none") +
  labs(
    title = "Tempo di esecuzione in funzione della Dimensione del Chunk",
    x = "Dimensione del Chunk",
    y = "Tempo Medio di Esecuzione [s]"
  )
```

La dimensione di default dei chunk, essendo il numero di cicli 2048 e 8 il numero di threads $\frac{2048}{8} = 255$ di chunk size.
Ma dai dati ottenuti si vede che all'aumentare della dimensione dei chunk il tempo di esecuzione anchesso aumenta con un punto di flesso intorno a valore di default (255).

## Risultati

Una volta ottimizzato il programma possiamo prendere i dati finali, in particolare si possono fare due tipi di test:

- **Numero di threads**: variamo il numero di threads in modo da vedere come scala 
- **Dimensione del problema**: variamo la dimensione del problema cosi' da vedere anch'esso come scala 
...
df_ryzen_stats %>%
  ...
  labs(
    title = "Tempo di Esecuzione in funzione del Numero di Thread",
    x = "Numero di Threads",
    y = "Tempo Medio di Esecuzione [s]",
    color = "Modalità"
  )
...
plot_efficiency <- ggplot(df_metrics, aes(x = Threads, y = Efficiency)) +
  ...
  scale_x_continuous(breaks = df_metrics$Threads) +
  labs(
    title = "Efficienza in Funzione del Numero di Thread",
    x = "Numero di Thread",
    y = "Efficienza",
    color = "Modalità"
  ) + 
...
Aumentare la dimensione del problema permette di vedere come il tempo di esecuzione scala, in particolare questa prova e' utile per capire se, ad esempio, il problema scala linearmente o esponenzialmente.
...
df_scale_stats %>%
  ...
  labs(
    title = "Tempo di Esecuzione in funzione del Numero di Iterazioni",
...
Nel caso dei frattali all'aumentare della dimensione del problema il tempo di esecuzione scala perfettamente linearmente.
Si nota anche che la pendenza della retta e' un indicatore delle prestazioni del porcessore, infatti dal grafico si vede come la CPU piu' vecchia (7600HQ) ha una pendenza piu' alta rispetto alla CPU piu' recente (1165G7).

## Cache

Essendo la differenza di velocita' fra memoria e cache e' sostanziale quindi per un processo andare ad attingere all'interno della memoria comporta una diminuzione nelle prestazioni del processo.D.
...
possiamo vedere che c'e' un minimo nel tempo di esecuzione della singola operazione intorno alla risoluzione di 1024 x 1024 che occupa all'interno della cache allincirca ~3 MB che sta bene all'interno della cache del processore usato per i test (12 MB).
...
Facendo il test con una seconda CPU con una dimensione della cache minore si ottengono i seguenti risultati:
...
df_cache_stats %>%
  ...
  labs(
    title = "Tempo per singola operazione: Confronto fra CPU", 
    x = "Altezza (Height)", 
    y = "Tempo Medio per Operazione [ns]",
    color = "CPU" 
  )
...
In alcuni casi il livello di parallelizzazione e le prestazioni ottenute dalla CPU possono essere insufficienti per alcune applicazioni, una alternativa puo' essere usare MPI su di un supercomputer solo che questa opzione e' difficilmente giustificabile, quindi una terza opzione e' quella di usare hardware specifico come possono essere le GPU.

Con l'avvento di **Nvidia** come una delle compagnie piu' grandi del pianeta non e' sbagliato pensare che i loro prodotti siano ovunque, quindi e' naturale che il linguaggio piu' usato per programmare su GPU sia **CUDA**.

Per quanto riguarda **Cuda** i suai vantaggi sono che:

- Elevate prestazioni
- Memoria condivisa
- Uso di linguaggi standard
- Compatibilita' su tutta la gamma di prodotti

Quindi per diletto possiamo confrontare i tempi ottenuti dalle due CPU con quelli ottenuti dalla GPU con lo stesso programma semplicemnte convertito in Cuda.
Non sono necessariamente equiparabili ma tutti e due le CPU usate sono quad core invece la GPU (Nvidia Gforce RTX 3060) ha la bellezza di **3584 cuda cores**.
...
Come si poteva prevedere la GPU in questo test batte nettamente le CPU, la questione e' che dipende dal tipo di calcolo che si sta facendo, nel caso dei frattali di Lyapunov viene eseguito essenzialmente il calcolo di un logaritmo un' innumerevole numero di volte quindi un caso affine alla GPU se il calcolo fosse qualcosa che richide elevata precisione, come ad esempio operazioni floting point, il distacco fra CPU e GPU sarebbe minore.

# Conlusioni 

Da tutti i test fatti non e' difficile notare che la parallelizzazione del codice porta ad una diminuzione del tempo di esecuzione sostanziale rispetto alla versione sequenziale.

Anche se OpenMP non permette di ottenere le assolute prestazini migliori dell'hardware, anche perche' esistono altri metodi che permettono un grado di parallelizzazione maggiore come puo' essere MPI o CUDA.
Il maggiore vantaggio che OpenMP ha rispetto alle altre opzione e' la facilit' d'uso, soprattutto per i non esperti, e la poca intrusivita' all'interno del programma.
Infatti nei casi piu' semplici, o nei problemi detti banalmente parallelizzabili, per rendere il codice parallelizzato basta giusto qualche riga di codice.