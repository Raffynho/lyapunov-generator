...
Color LyapunovGenerator::getColor(double lambda) const {
    if (lambda >= 0) return {0, 0, 0}; // Caos = Nero
...
// === METODO SEQUENZIALE ===
void LyapunovGenerator::generateSequential() {
...
// === METODO PARALLELO (OpenMP) ===
void LyapunovGenerator::generateParallel() {
    size_t seq_len = sequence.length();
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            double lambda = sum_log / lyap_steps;
            // image è condiviso, ma thread-safe poiché ogni thread scrive in un indice unico
            image[y * width + x] = getColor(lambda);
        }
    }
}
